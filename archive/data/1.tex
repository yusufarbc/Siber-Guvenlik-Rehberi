...
\section*{Giris}
Bu bolum, kitabin geri kalaninda kullanilan tum teknik bolumler icin yonetsel ve mimari temeli olusturur.
Amac, guvenligi yalnizca bir "saldiri-savunma" konusu olarak degil; kurumsal hedefler, butce, mevzuat,
operasyon surekliligi ve insan faktoru ile birlikte ele alan bir \textit{cozum mimarisi} cercevesi sunmaktir.

\Not{Bu bolumde tanimlanan yonetisim, risk, uyum ve sureklilik modelleri; SOC, IAM, bulut, DevSecOps ve olay mudahale bolumlerindeki tum kararlarin referans noktasi olarak kullanilmalidir.}

\section{Bilgi Guvenliginin Temelleri}

\subsection{CIA Uclusu (Gizlilik, Butunluk, Kullanilabilirlik)}
Bilgi guvenliginin temel modeli \textbf{CIA Uclusu}'dur. Kurumsal mimarilerde bu model, teknik kontrollerin
onceliklendirilmesi ve denetlenebilirligin saglanmasi icin minimum ortak dildir.

\begin{itemize}
    \item \textbf{Gizlilik (Confidentiality):} Yetkisiz kisilerin verilere erisiminin engellenmesi.
    Tipik kontroller: sifreleme, veri siniflandirma, erisim politikasi, DLP.
    \item \textbf{Butunluk (Integrity):} Verinin dogrulugu ve degistirilmezliginin korunmasi.
    Tipik kontroller: imza/hashing, degisiklik yonetimi, kayit butunlugu, denetim izi.
    \item \textbf{Kullanilabilirlik (Availability):} Yetkili kullanicilarin sistemlere zamaninda erisebilmesi.
    Tipik kontroller: yedeklilik, kapasite planlama, felaket kurtarma, anti-DDoS mekanizmalari.
\end{itemize}

\subsubsection{CIA Ihlalinin Is Etkisi}
Kurumsal ortamlarda tek bir olay birden fazla ilkeyi ayni anda ihlal edebilir.
Ornegin fidye yazilimi saldirisi:
\begin{enumerate}
    \item Veriyi okunamaz hale getirerek \textbf{kullanilabilirligi} bozar,
    \item Veri sizdirmasi ile \textbf{gizliligi} ihlal eder,
    \item Log veya yedekleri manipule ederek \textbf{butunlugu} bozar.
\end{enumerate}

\subsection{Kurumsal Dijital Donusumde Guvenlik Yaklasimi}
Bulut, uzaktan calisma, SaaS ve API tabanli servisler; guvenligin tek bir cihaza veya tek bir perimeter
duvarina dayali yurutulemeyecegini gostermistir. Bu nedenle modern kurumsal yaklasim:

\begin{itemize}
    \item \textbf{Risk tabanli planlama} (NIST CSF, ISO 27001),
    \item \textbf{Kimlik merkezli erisim} (IAM, MFA, PAM, Zero Trust),
    \item \textbf{Surekli izleme ve olcum} (SIEM/SOAR, KPI/KRI),
    \item \textbf{Uyumla birlikte tasarim} (KVKK, GDPR, 5651) prensiplerini birlestirir.
\end{itemize}

\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    Placeholder: \texttt{img/ch1-dijital-donusum-guvenlik-mimarisi.png}\\
    (Kurumsal dijital donusumde guvenlik katmanlari: Kimlik, Veri, Ag, Uygulama, Operasyon)}}
    \caption{Kurumsal dijital donusumde guvenlik mimarisi referans modeli}
    \label{fig:ch1-digital-security-architecture}
\end{figure}

\section{Siber Guvenlik Stratejisi ve Kurumsal Mimari}

\subsection{Guvenlik Yatirimlarinda Maliyet ve Performans (TCO) Analizi}
Teknik kabiliyetin yuksek olmasi tek basina dogru urun secimi anlamina gelmez. Kurumsal secimlerde
\textbf{Toplam Sahip Olma Maliyeti (TCO)} yaklasimi zorunludur.

\begin{table}[htbp]
\centering
\caption{Guvenlik cozumu seciminde TCO degerlendirme matrisi}
\begin{tabular}{|p{3.2cm}|p{4.6cm}|p{6.2cm}|}
\hline
\textbf{Kalem} & \textbf{Degerlendirme Sorusu} & \textbf{Notlar} \\
\hline
Lisans ve abonelik & Ilk alim ve yenileme maliyeti nedir? & Uc yillik periyotla hesaplanmalidir. \\
\hline
Operasyonel yuk & Ekip bu urunu isletebilecek mi? & Egitim, personel sayisi, vardiya yukleri dahil edilmelidir. \\
\hline
Entegrasyon maliyeti & SIEM, IAM, ticketing ile uyumlu mu? & API, log formati, agent ihtiyaci ve migration eforu olculmelidir. \\
\hline
Performans etkisi & Son kullanici/altyapi performansina etkisi nedir? & Ozellikle EDR, DLP, inline ag cihazi etkileri test edilmelidir. \\
\hline
Risk azaltim etkisi & Hangi tehdidi ne oranda azaltir? & MTTD/MTTR ve olay frekansi uzerinden olculmelidir. \\
\hline
\end{tabular}
\end{table}

\Ipucu{TCO analizi, yalnizca satin alma ekibi tarafindan degil; guvenlik, sistem, ag, uygulama ve finans ekiplerinin ortak calismasi ile yapilmalidir.}

\subsection{Uretici Konsolidasyonu (Vendor Consolidation) Stratejileri}
Kurumsal ortamlarda asiri urun cesitliligi, gozlemlenebilirlik kaybina ve operasyonel maliyet artisina yol acar.
Ancak kontrolsuz konsolidasyon da tek urun bagimliligi riski yaratir.

\begin{enumerate}
    \item \textbf{Fonksiyonel cakisma analizi:} Ayni isi yapan urunlerin (ornegin birden cok EDR ajaninin) tespiti.
    \item \textbf{Kritik bagimlilik analizi:} Tek urune baglilik durumunda olasi kesinti etkisinin olculmesi.
    \item \textbf{Asamali gecis plani:} Once pilot, sonra bolgesel, en son kurum geneli gecis.
    \item \textbf{Cikis stratejisi:} Her urun icin veri tasinabilirligi ve geri donus plani.
\end{enumerate}

\Uyari{"Tek panel" hedefi dogru olmakla birlikte, tek ureticiye tam baglilik is surekliligi ve fiyatlama riskini arttirabilir. Kritik alanlarda ikinci secenek stratejisi korunmalidir.}

\section{Yonetisim, Risk ve Uyumluluk (GRC)}

\subsection{Risk Degerlendirme Metodolojileri ve Tehdit Modelleme}
Kurumsal risk yonetimi, sadece teknik zafiyet taramasi degil; is etkisi, operasyonel bagimlilik ve
yasal yaptirim boyutlarini da kapsar.

\subsubsection{Temel metodolojiler}
\begin{itemize}
    \item \textbf{NIST CSF:} Identify, Protect, Detect, Respond, Recover fonksiyonlari ile yonetsel omurga.
    \item \textbf{ISO 27001/27005:} BGYS ve risk isleme yaklasimi.
    \item \textbf{FAIR:} Riskin finansal olarak nicellestirilmesi (ALE, LEF, LM).
    \item \textbf{OCTAVE:} Is hedefleri odakli kurumsal risk gorusu.
\end{itemize}

\subsubsection{Pratik tehdit modelleme akisi}
\begin{enumerate}
    \item Kritik varliklarin tanimlanmasi (kimlik verisi, odeme verisi, OT varliklari).
    \item Saldiri yuzeyinin ve guven sinirlarinin cikarilmasi.
    \item Tehdit senaryolarinin olasilik-etki bazinda puanlanmasi.
    \item Kontrol secimi ve risk isleme karari (kabul, azalt, transfer, kacın).
\end{enumerate}

\subsection{Yasal Regulasyonlar: KVKK, GDPR ve Uyum Surecleri}
KVKK ve GDPR, yalnizca hukuk biriminin konusu degildir; veri akislari, log yonetimi, erisim kontrolu ve
olay bildirimi sureclerine teknik duzeyde yansir.

\begin{itemize}
    \item \textbf{KVKK:} Acik riza, veri minimizasyonu, veri isleme amaci, saklama-imha politikasi.
    \item \textbf{GDPR:} Veri sahibi haklari, 72 saat ihlal bildirimi, DPIA, sinir otesi veri transfer kontrolleri.
\end{itemize}

\subsubsection{Uyum sureci icin minimum teknik kontrol listesi}
\begin{itemize}
    \item Envanterlenmis veri siniflandirma modeli,
    \item Sifreleme ve anahtar yonetimi,
    \item Erisim kayitlari ve denetim izi,
    \item Veriye erisimde yetki ayrimi (SoD) ve en az ayricalik,
    \item Ihlal bildirimi ve olay mudahale entegrasyonu.
\end{itemize}

\subsection{Turkiye'de 5651 Sayili Kanun ve Yasal Loglama Yukumlulukleri}
5651 kapsaminda kurumlarin, erisim ve islem kayitlarini mevzuata uygun sekilde tutmasi;
zaman damgasi butunlugu, log saklama suresi ve delil niteligi acisindan kritiktir.

\begin{table}[htbp]
\centering
\caption{Yasal loglama mimarisi icin onerilen kontrol katmanlari}
\begin{tabular}{|p{3.4cm}|p{5.2cm}|p{5.4cm}|}
\hline
\textbf{Katman} & \textbf{Kontrol} & \textbf{Mimari Not} \\
\hline
Kaynak sistemler & Sunucu, ag cihazi, uygulama loglari & Log kapsam matrisi ile zorunlu olaylar standartlastirilmalidir. \\
\hline
Toplama & Merkezi log toplama/SIEM (ornegin Wazuh, Splunk) & NTP senkronizasyonu ve butunluk kontrolu zorunludur. \\
\hline
Saklama & Degistirilemez depolama ve arsivleme & Delil zinciri ve saklama suresi politikalari ile uyumlu olmalidir. \\
\hline
Raporlama & Denetim ve adli surecler icin rapor setleri & Yetkili birimlere rol bazli erisim verilmelidir. \\
\hline
\end{tabular}
\end{table}

\section{Is Surekliligi ve Kriz Yonetimi}

\subsection{Is Etki Analizi (BIA - Business Impact Analysis)}
BIA, hangi surecin ne kadar sure durabilecegini ve veri kaybina ne kadar tolerans oldugunu
somutlastirir. BIA olmadan RTO/RPO hedefleri gercekci belirlenemez.

\begin{enumerate}
    \item Kritik is sureclerinin belirlenmesi,
    \item Surec-bagimlilik haritasi (uygulama, veri tabani, ag, insan kaynagi, tedarikci),
    \item Finansal, operasyonel ve itibari etki modellemesi,
    \item RTO/RPO hedeflerinin yonetim onayli hale getirilmesi.
\end{enumerate}

\subsection{Is Surekliligi Planlamasi (BCP) ve Felaket Kurtarma}
\textbf{BCP} is operasyonlarinin devamini, \textbf{DR} ise IT sistemlerinin geri kaldirilmasini hedefler.
Iki planin ayrik degil, birbirine bagli calismasi gerekir.

\begin{itemize}
    \item BCP tarafinda: surec alternatifleri, manuel operasyon adimlari, kriz iletisim plani.
    \item DR tarafinda: yedekleme stratejisi, otomasyon scriptleri, geri donus testleri, tatbikat takvimi.
\end{itemize}

\subsection{Alternatif Veri Merkezi Stratejileri (Hot, Warm ve Cold Siteler)}

\begin{table}[htbp]
\centering
\caption{Alternatif veri merkezi modelleri karsilastirmasi}
\begin{tabular}{|p{2.6cm}|p{3.5cm}|p{3.0cm}|p{5.1cm}|}
\hline
\textbf{Model} & \textbf{Kurtarma Hiz} & \textbf{Maliyet} & \textbf{Tipik Kullanim Senaryosu} \\
\hline
Hot Site & Dakikalar & Yuksek & Finans, e-ticaret, 7/24 kritik operasyonlar \\
\hline
Warm Site & Saatler & Orta & Kurumsal ERP/CRM, planli geri donus gerektiren yapilar \\
\hline
Cold Site & Gunler & Dusuk & Kritikligi dusuk islevler, regulator minimum gereklilik \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    Placeholder: \texttt{img/ch1-bcp-dr-topology.png}\\
    (Primary DC, DR Site, backup repository ve failover akis semasi)}}
    \caption{BCP/DR topolojisi ve failover akis taslagi}
    \label{fig:ch1-bcp-dr-topology}
\end{figure}

\section{Siber Guvenlik Politikalari ve Insan Faktoru}

\subsection{Son Kullanici Farkindaligi ve Egitim Planlamasi}
Kurumsal ihlallerin onemli bir kismi, teknik kontrolden cok insan hatasi kaynaklidir.
Bu nedenle farkindalik programi "yilda bir egitim" seviyesinde kalmamali; rol bazli ve olculebilir olmalidir.

\begin{itemize}
    \item \textbf{Rol bazli egitim}: Son kullanici, sistem yoneticisi, gelistirici, SOC analisti icin farkli icerik,
    \item \textbf{Mikro egitim yaklasimi}: Kisa, periyodik, senaryo temelli moduller,
    \item \textbf{Olcumleme}: Tiklama orani, raporlama orani, tekrar hata frekansi gibi KPI'lar.
\end{itemize}

\subsection{Oltalama (Phishing) Simulasyon Kampanyalarinin Yonetimi}
Phishing simulasyonu, personeli cezalandirma degil; risk gorunurlugunu artirma ve davranis degisikligi
uretme araci olarak konumlandirilmalidir.

\subsubsection{Asamali kampanya modeli}
\begin{enumerate}
    \item \textbf{Planlama}: Hedef grup, senaryo ve basari metriklerinin tanimlanmasi.
    \item \textbf{Uygulama}: Kontrollu ve kayitli simule e-posta gonderimi.
    \item \textbf{Olcumleme}: Tiklama, kimlik bilgisi girisi ve bildirim oranlarinin analizi.
    \item \textbf{Iyilestirme}: Yukseltici egitim ve yonetsel aksiyonlarin tanimlanmasi.
\end{enumerate}

\begin{quote}
\textbf{Best Practice:} Simulasyon sonuclarini bireysel cezalandirma amaciyla degil,
organizasyonel risk azaltma ve egitim olgunlugu artirma amaciyla degerlendirin.
\end{quote}

\subsubsection{Kurumsal olcum panosu icin ornek KPI seti}
\begin{itemize}
    \item Phishing e-postasina tiklama orani,
    \item Supheli e-postayi SOC'a bildirme orani,
    \item Yanlis pozitif bildirim orani,
    \item MTTD/MTTR ile korele egitim etkisi,
    \item Departman bazli tekrar risk skoru.
\end{itemize}

\section*{Bolum Sonu Degerlendirme}
Bu bolumde kurumsal guvenligin yonetsel omurgasi tanimlanmistir:
\begin{itemize}
    \item CIA modelinden is etkisine uzanan temel guvenlik ilkeleri,
    \item TCO ve vendor konsolidasyonu ile stratejik mimari kararlar,
    \item KVKK, GDPR ve 5651 ile uyumlu GRC yaklasimi,
    \item BIA/BCP/DR ile sureklilik ve kriz hazirligi,
    \item Insan faktorune dayali farkindalik ve phishing yonetimi.
\end{itemize}

Sonraki bolumde fiziksel guvenlik katmani, tesis ve veri merkezi perspektifinde ele alinacaktir.
