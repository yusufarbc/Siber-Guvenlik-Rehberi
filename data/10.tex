...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|X|X|}
        \hline
        \rowcolor{tableheadcolor}
        \textbf{Framework} & \textbf{Kapsam} & \textbf{Temel Gereksinimler}  \\
        \hline
        ISO 27001 & Bilgi güvenliği yönetim sistemi & 114 kontrol, risk yönetimi, sürekli iyileştirme  \\
        \hline
        SOX & Finansal raporlama & IT kontrolleri, veri bütünlüğü, erişim kontrolleri  \\
        \hline
        GDPR & Kişisel veri koruma & Veri minimizasyonu, izin yönetimi, ihlal bildirimi  \\
        \hline
        KVKK & Kişisel veri koruma & Veri işleme kayıtları, DPO atama, etki analizi  \\
        \hline
        HIPAA & Sağlık verisi koruma & Şifreleme, erişim kontrolü, denetim kayıtları  \\
        \hline
        PCI DSS & Ödeme kartı verisi & Ağ segmentasyonu, şifreleme, penetrasyon testleri  \\
        \hline
        NIST CSF & Siber güvenlik çerçevesi & Belirle, koru, tespit et, müdahale et, kurtar  \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{itemize}
    \item \textbf{Senaryo:} Ayrıcalıklı bir çalışanın, hassas müşteri verilerini içeren dizüstü bilgisayarının çalınması.
    \item \textbf{Adım 1: Değerleme (Varlık Değeri):} Varlığın değeri, cihazın fiziksel maliyeti ve içerdiği hassas verilerin (PII, entelektüel mülkiyet vb.) toplamıdır. Örneğin: \$2.500 (laptop) + \$22.500 (veri ve itibar kaybı) = \textbf{\$25.000}.
    \item \textbf{Adım 2: Tek Kayıp Beklentisi (SLE):} SLE = Varlık Değeri $\times$ Maruz Kalma Faktörü. Şifrelenmemiş veriye sahip bir laptop çalındığında maruz kalma faktörü \%100'dür. SLE: \$25.000 $\times$ 1 = \$25.000.
    \item \textbf{Adım 3: Yıllık Meydana Gelme Oranı (ARO):} Geçmiş olaylara göre yılda ortalama 11 laptop hırsızlığı yaşanıyorsa, ARO = 11.
    \item \textbf{Adım 4: Yıllık Kayıp Beklentisi (ALE):} ALE = SLE $\times$ ARO. Yani \$25.000 $\times$ 11 = \$275.000.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|X|X|}
        \hline
        \textbf{İş Kritikliği Seviyesi} & \textbf{Örnek İş Süreci} & \textbf{RTO (Kurtarma Süresi)} \\
        \hline
        \textbf{Görev-Kritik (Mission-Critical)} & Müşteri Sipariş Sistemi, Finansal İşlemler & Saniyelerden Dakikalara \\
        \hline
        \textbf{Temel (Essential)} & E-posta, İK Uygulamaları & 4 ila 24 saat \\
        \hline
        \textbf{Kritik Olmayan (Nonessential)} & İç Raporlama Araçları, İdari Dosyalama & 24 saatten Haftalara \\
        \hline
    \end{tabularx}
\end{table}
